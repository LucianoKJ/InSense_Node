...
router.get("/ordersdetail", async (req, res) => {
    ...
    if (output.logInStatus) {
        const sql =
            "SELECT `OrderTb`.`orderId`,`OrderTb`.`orderStatus`,`OrderTb`.`itemInfo`,`users`.`userFirstName`,`users`.`userLastName`,`users`.`userPostCode`,`users`.`userCity`,`users`.`userDistrict`,`users`.`userAddress` FROM `OrderTb` INNER JOIN `users` ON `OrderTb`.`userId` = `users`.`userId`";
